...
const tests = [
  {
    ...
    check: (body, headers) => {
      ...
      if (n !== 59) throw new Error(`gated_routes=${n}, expected 59`);
      return `59 routes, network=${j.network}, pay_to_suffix=${j.pay_to_suffix}`;
    },
  },
  {
    ...
    check: (body) => {
      ...
      if (j.entry_count !== 59) throw new Error(`entry_count=${j.entry_count}`);
